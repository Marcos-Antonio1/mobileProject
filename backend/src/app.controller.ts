...
export class AppController {
  ...
  @Post('/editdata')
   editdata(@Body() body){
    ...
    const index = buyer.indexOf(user)
    if(user){
      const newData={
        id:body.id,
        nome:body.nome,
  ...
  confirm(@Body() body){
     orders.forEach(function(element:any,index:number){
  ...
  @Get('/hortiFruit')
   hortiFruit(){
      return EstabelecimentoHortiFruit;
   }

   @Get('/acouque')
   acouque(){
      return EstabelecimentoAcouque;
   }
   
   @Get('/agua')
   agua(){
      return EstabelecimentoAgua;
   }
  
   @Get('/padaria')
   padaria(){
      return EstabelecimentoPadaria;
   }


  serachBuyer(id){
    let user= null;
    buyer.forEach(function(element,i){
      if(element.id == id){
        user= element;
      }
    })
